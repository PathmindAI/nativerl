    ...
    public String generatePythonTrainer() {
        ...
        String trainer = "import glob, gym, nativerl, ray, sys, os, random\n"
            ...
            + (multiAgent
                ...
                + "\n"
                + "    def getMetrics(self):\n"
                + "        return self.nativeEnv.getMetrics()\n"
                + "\n"
                + "class MyCallbacks(DefaultCallbacks):\n"
                + "    def on_episode_start(self, worker: RolloutWorker, base_env: BaseEnv,\n"
                + "                         policies: Dict[str, Policy],\n"
                + "                         episode: MultiAgentEpisode, **kwargs):\n"
                + "        print(\"kepricondebug : episode {} started\".format(episode.episode_id))\n"
                + "\n"
                + "    def on_episode_step(self, worker: RolloutWorker, base_env: BaseEnv,\n"
                + "                        episode: MultiAgentEpisode, **kwargs):\n"
                + "        #print(\"kepricondebug : agent_rewards : \" +  str(episode.agent_rewards))\n"
                + "        #print(\"kepricondebug : last_raw_obs_for : \" + str(episode.last_raw_obs_for()))\n"
                + "        #print(\"kepricondebug : lasT_observation_for : \" + str(episode.last_observation_for()))\n"
                + "        #print(\"kepricondebug : prev reward : \" + str(episode.prev_reward_for()))\n"
                + "        a=3\n"
                + "\n"
                + "    def on_episode_end(self, worker: RolloutWorker, base_env: BaseEnv,\n"
                + "                       policies: Dict[str, Policy], episode: MultiAgentEpisode,\n"
                + "                       **kwargs):\n"
                + "        print(\"kepricondebug : logdir {} \".format(worker.io_context.log_dir))\n"
                + "        print(\"kepricondebug : episode.reward: {}\".format(episode.agent_rewards))\n"
                + "        print(\"kepricondebug : episode.length : {}\".format(episode.length))\n"
                + "        print(\"kepricondebug : episode {} end\".format(episode.episode_id))\n"
                + "        print(\"kepricondebug : {} \".format(worker.env.getMetrics()))\n"
                + "\n"
            ...
            + "\n"
            + "ray.init(webui_host='127.0.0.1', log_to_driver=" + (userLog ? "True" : "False") + ")\n"
