    ...
    public String generatePythonTrainer() {
        ...
        String trainer = "import glob, gym, nativerl, ray, sys, os, random\n"
            ...
            + (multiAgent
                ...
                + "\n"
                + "        # Stopping criteria at iteration 1\n"
                + "        self.too_many_steps = False # Max steps\n"
                + "        # Stopping criteria at iteration self.early_check\n"
                ...
                + "        self.max_steps = 200000 # Remove with +9999999\n"
                + "\n"
                ...
                + "            self.too_many_steps = result['timesteps_this_iter'] > self.max_steps\n"
                ...
                + "            self.episode_reward_range = np.max(np.array(self.episode_reward_window[-25:])) - np.min(np.array(self.episode_reward_window[-25:]))\n"
                + "            # Episode reward mean activity\n"
                + "            self.episode_reward_mean = np.mean(np.array(self.episode_reward_window[-25:]))\n"
                + "            self.episode_reward_mean_latest = np.mean(np.array(self.episode_reward_window[-5:]))\n"
                + "\n"
                + "            # Convergence check\n"
                + "            if (np.abs(self.episode_reward_mean_latest - self.episode_reward_mean) / np.abs(self.episode_reward_mean) < self.episode_reward_range_threshold) and (np.abs(self.episode_reward_range) < np.abs(np.mean(np.array(self.episode_reward_window[-25:])) * 2)):\n"
                + "                self.converged = True\n"
                + "\n"
