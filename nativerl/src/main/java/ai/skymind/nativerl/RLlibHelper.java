    ...
    public String generatePythonTrainer() {
        ...
        String trainer = "import glob, gym, nativerl, ray, sys, os, random\n"
            ...
            + (multiAgent
                ...
                + "        self.vf_pred_adequate_window = []\n"
                + "        self.vf_pred_mean = 0\n"
                + "        self.vf_pred_mean_latest = 0\n"
                + "\n"
                + "        # Configs\n"
                + "        self.episode_reward_range_threshold = 0.05 # Remove with 0\n"
                + "        self.entropy_slope_threshold = 0.01 # Remove with -9999999\n"
                + "        self.vf_loss_range_threshold = 0.1 # Remove with 0\n"
                + "        self.value_pred_range_threshold = 0.05 # Remove with 0\n"
                + "        self.value_pred_adequate_threshold = 0.75 # Remove with 1\n"
                + "        self.value_pred_great_threshold = 0.95 # Remove with 1\n"
                + "\n"
                ...
                + "        self.vf_pred_adequate_window.append(result['info/learner/default_policy/vf_explained_var'])\n"
                + "\n"
                ...
                + "        if result['training_iteration'] == 25:\n"
                ...
                + "            self.value_pred_great = (np.abs(result['info/learner/default_policy/vf_explained_var']) > self.value_pred_great_threshold)\n"
                ...
                + "            if not self.should_stop and (self.no_converge_risk or self.no_discovery_risk or self.value_pred_great):\n"
                + "                self.should_stop = True\n"
                + "                return self.should_stop\n"
                + "\n"
                + "        # Convergence Filter\n"
                ...
                + "            self.episode_reward_mean = np.mean(np.array(self.episode_reward_window[-100:]))\n"
                + "            self.episode_reward_mean_latest = np.mean(np.array(self.episode_reward_window[-20:]))\n"
                + "            # Value function activity\n"
                + "            self.vf_pred_mean = np.mean(np.array(self.vf_pred_adequate_window[-25:]))\n"
                + "            self.vf_pred_mean_latest = np.mean(np.array(self.vf_pred_adequate_window[-5:]))\n"
                + "            # Explained variance activity\n"
                + "            self.value_pred_adequate = (np.abs(result['info/learner/default_policy/vf_explained_var']) > self.value_pred_adequate_threshold)\n"
                + "\n"
                ...
                + "            if (np.abs(self.vf_pred_mean_latest - self.vf_pred_mean) / np.abs(self.vf_pred_mean) < self.value_pred_range_threshold) and self.value_pred_adequate:\n"
