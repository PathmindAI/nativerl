    ...
    public String generatePythonTrainer() {
        ...
        String trainer = "import glob, gym, nativerl, ray, sys, os, random\n"
            ...
            + (multiAgent
                ...
                + "    def __init__(self):\n"
                + "        # core attributes\n"
                + "        self.should_stop = False # criteria met\n"
                + "        self.too_many_iter = False # iteration limit\n"
                + "        self.too_much_time = False # time limit\n"
                + "        self.too_much_reward = False # double ceiling\n"
                + "\n"
                + "        # experimental attributes\n"
                + "        # initial stopping - at iterations self.initial_check\n"
                + "        self.too_many_steps = False # too many episode steps\n"
                + "\n"
                + "        # early stopping - at iteration self.early_check\n"
                + "        self.no_discovery_risk = False # value loss never changes\n"
                + "        self.no_converge_risk = False # entropy never drops\n"
                + "\n"
                + "        # convergence signals - each iteration from self.converge_check onward\n"
                + "        self.converged = False # slope flattened and self.value_pred_adequate = True\n"
                ...
                + "        self.episode_reward_slope = 0\n"
                + "        self.episode_reward_slope_later = 0\n"
                + "        self.episode_reward_slope_latest = 0\n"
                + "\n"
                + "        # entropy behaviour\n"
                ...
                + "        self.vf_loss_start = 0\n"
                + "        self.vf_loss_now = 0\n"
                + "        self.vf_loss_slope = 0\n"
                + "\n"
                + "        # configs\n"
                + "        self.reward_slope_threshold = 0.1 # remove with +9999999\n"
                + "        self.entropy_slope_threshold = 0.01 # remove with -9999999\n"
                + "        self.vf_loss_slope_threshold = 0.1 # remove with +9999999\n"
                + "        self.value_pred_threshold = 0.5 # remove with -9999999\n"
                + "        self.initial_check = 1 # remove with -1\n"
                + "        self.early_check = 50 # remove with -1\n"
                + "        self.converge_check = 100 # remove with -1\n"
                + "        self.step_check = 100000 # remove with +9999999\n"
                + "\n"
                ...
                + "            self.vf_loss_start = result['info/learner/default_policy/vf_loss']\n"
                + "            self.too_many_steps = result['timesteps_this_iter'] > self.step_check\n"
                ...
                + "            self.vf_loss_now = result['info/learner/default_policy/vf_loss']\n"
                + "            self.episode_reward_slope = self.episode_reward_window[-1] - self.episode_reward_window[0]\n"
                + "            self.entropy_slope = self.entropy_now - self.entropy_start\n"
                + "            self.vf_loss_slope = self.vf_loss_now - self.vf_loss_start\n"
                + "            if np.abs(self.episode_reward_slope) < np.abs(self.episode_reward_slope * self.reward_slope_threshold):\n"
                + "                self.no_converge_risk = True\n"
                + "            elif self.entropy_slope > np.abs(self.entropy_start * self.entropy_slope_threshold):\n"
                + "                self.no_converge_risk = True\n"
                + "            if np.abs(self.vf_loss_slope) < np.abs(self.vf_loss_start * self.vf_loss_slope_threshold):\n"
                ...
                + "\n"
                + "            # convergence trends\n"
                + "            if result['training_iteration'] > early_check:\n"
                + "                self.episode_reward_window.append(result['info/learner/default_policy/entropy'])\n"
                + "\n"
                + "            # convergence filter\n"
                + "            if result['training_iteration'] >= self.converge_check:\n"
                + "                self.episode_reward_slope = self.episode_reward_window[-1] - self.episode_reward_window[0]\n"
                + "                self.episode_reward_slope_later =  self.episode_reward_window[-1] - self.episode_reward_window[2]\n"
                + "                self.episode_reward_slope_latest = self.episode_reward_window[-1] - self.episode_reward_window[-5]\n"
                + "                if (np.abs(self.episode_reward_slope) < np.abs(self.episode_reward_window[0] * self.reward_slope_threshold))"
