...
aws s3 cp s3://dev-training-dynamic-files.pathmind.com/model_file/1479 model.zip > /dev/null ;
...
export REWARD_SNIPPET='reward += -1.5;
reward -= (after[0] - before[0])*2;
reward -= (after[1] - before[1])*1;' ;
export METRICS_SNIPPET='' ;
export DISCRETE_ACTIONS='8' ;
export CONTINUOUS_OBSERVATIONS='551' ;
