...
aws s3 cp s3://${ENVIRONMENT}-training-static-files.pathmind.com/nativerl/pr_test/nativerl-1.2.0-SNAPSHOT-bin.zip nativerl-1.2.0-SNAPSHOT-bin.zip > /dev/null ;
...
mkdir -p work ;
cd work ;
unzip ../nativerl-1.2.0-SNAPSHOT-bin.zip > /dev/null ;
...
export REWARD_SNIPPET='reward += after[0] - 0.1;' ;
export METRICS_SNIPPET='' ;
export DISCRETE_ACTIONS='2' ;
export CONTINUOUS_OBSERVATIONS='3' ;
