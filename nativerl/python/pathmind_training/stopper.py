...
class Stopper:
    ...
    def stop(self, trial_id, result):
        ...
        if self.too_many_iter:
            write_file(
                ["Early Stop Reason: Max Iterations Limit: {}".format(str(trial_id))],
        ...
        if self.too_much_time:
            write_file(
                ["Early Stop Reason: Max Train Time Limit: {}".format(str(trial_id))],
                "ExperimentCompletionReport.txt",
                self.output_dir,
                self.algorithm,
            )
        ...
        if self.too_many_episodes:
            write_file(
                ["Early Stop Reason: Max Episode Limit: {}".format(str(trial_id))],
                "ExperimentCompletionReport.txt",
                self.output_dir,
                self.algorithm,
            )
        ...
        if result["training_iteration"] == 50:
            ...
            self.vf_loss_range = np.max(np.array(self.vf_loss_window)) - np.min(
                np.array(self.vf_loss_window)
            )

            if self.entropy_slope > np.abs(
                self.entropy_start * self.entropy_slope_threshold
            ):
                self.no_converge_risk = True
            if np.abs(self.vf_loss_range) < np.abs(
                self.vf_loss_window[0] * self.vf_loss_range_threshold
            ):
            ...
            if self.no_converge_risk or self.no_discovery_risk:
                write_file(
                    ["Early Stop Reason: No Learning Detected"],
                    "ExperimentCompletionReport.txt",
                    self.output_dir,
                    self.algorithm,
                )
        ...
        if result["training_iteration"] >= self.convergence_check_start_iteration:
            ...
            self.episode_reward_range = np.max(
                np.array(self.episode_reward_window[trial_id][-50:])
            ) - np.min(np.array(self.episode_reward_window[trial_id][-50:]))

            # Episode reward mean activity
            self.episode_reward_mean = np.mean(
                np.array(self.episode_reward_window[trial_id][-75:])
            )
            self.episode_reward_mean_latest = np.mean(
                np.array(self.episode_reward_window[trial_id][-15:])
            )
            ...
            reward_level = np.abs(
                self.episode_reward_mean_latest - self.episode_reward_mean
            ) / np.abs(self.episode_reward_mean)
            reward_window = np.abs(
                np.mean(np.array(self.episode_reward_window[trial_id][-50:])) * 2
            )
            if (reward_level < self.episode_reward_range_threshold) and (
                np.abs(self.episode_reward_range) < reward_window
            ):
                self.episode_reward_converged = True

            # Explained variance leveled off
            variance = np.abs(self.vf_pred_mean_latest - self.vf_pred_mean) / np.abs(
                self.vf_pred_mean
            )
            ...
            if self.episode_reward_converged and self.value_pred_converged:
                write_file(
                    [
                        "Early Stop Reason: Training has converged : {}".format(
                            str(trial_id)
                        )
                    ],
                    "ExperimentCompletionReport.txt",
