...
def buildNativerl(image_name) {
    ...
    sh "aws s3 cp ${WORKSPACE}/nativerl/target/nativerl-1.7.2-SNAPSHOT-bin.zip s3://${env.BRANCH_NAME}-training-static-files.pathmind.com/nativerl/1_7_2/nativerl-1.7.2-SNAPSHOT-bin.zip"
    sh "aws s3 cp ${WORKSPACE}/nativerl/target/nativerl-1.7.2-SNAPSHOT-bin.zip s3://${env.BRANCH_NAME}-model-analyzer-static-files.pathmind.com/nativerl-bin.zip"
}
